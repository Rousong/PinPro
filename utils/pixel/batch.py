...
auth = coreapi.auth.BasicAuthentication("admin", "qq111111")  # 上传需要用户验证 填写你的用户名和密码
client = coreapi.Client(auth=auth)
schema = client.get("https://pixel.do1024.com/api/v2/docs")  # API文档
...
for url in upload_url:
    ...
    params["url"] = url
    pinResult = client.action(schema, action, params=params,transform=transform)
    print(pinResult)
