...
auth = coreapi.auth.BasicAuthentication("hentai", "qq11111")  # 上传需要用户验证 填写你的用户名和密码
client = coreapi.Client(auth=auth)
schema = client.get("https://2zzy.com/api/v2/docs")  # API文档

ADD_TO_BOARD = False  # 是否加分类的开关 开是True 关闭则是False
BOARD_ID = 6 # 要添加到哪个分类ID下
